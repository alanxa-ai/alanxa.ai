...
const CloudinaryImageUpload = ({ onUploadSuccess, folder = 'alanxa' }) => {
  ...
  return (
    <div className="cloudinary-upload w-full">
      {!preview ? (
        <div className="border-2 border-dashed border-gray-300 bg-gray-50 rounded-xl p-8 text-center hover:border-blue-500 hover:bg-blue-50/50 transition-all duration-300 group">
            ...
            <div className="flex flex-col items-center">
              {uploading ? (
                ...
                <>
                  ...
                  <p className="text-gray-900 font-bold mb-1">Click to upload image</p>
                  <p className="text-gray-500 text-sm">PNG, JPG, GIF (Max 5MB)</p>
                  <div className="mt-4 px-6 py-2 bg-white border border-gray-200 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
                      Choose File
                  </div>
                </>
              )}
        ...
        <div className="relative group rounded-xl overflow-hidden border border-gray-200">
          ...
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-start justify-end p-2">
               <button
                ...
                className="p-2 bg-red-500 text-white rounded-full hover:bg-red-600 transition shadow-lg transform hover:rotate-90 duration-300"
              >
                <X className="w-5 h-5" />
              </button>
          </div>
      ...
      {preview && !uploading && !error && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
          <div className="p-1 bg-green-100 rounded-full">
             <ImageIcon className="w-4 h-4 text-green-600" />
          </div>
          <p className="text-green-700 text-sm font-bold">Image ready</p>
        </div>
      )}
    </div>
  );
